/*<<<<<COPYLEFT*/
/** @file factufrmeditalbaranventadet.cpp Fichero de edición de detalles de albaranes de venta
...
// FIELD Notas text
// TYPE FrmEditRecDetail factu::AlbaranVentaDet AlbaranDet Venta
/*>>>>>MODULE_INFO*/
...
/*<<<<<FRMEDITALBARANVENTADET_CONSTRUCTOR*/
FrmEditAlbaranVentaDet::FrmEditAlbaranVentaDet(FrmEditRecMaster *frmmaster, int ndetail,
...
{
	if ( !name )
	    setName( "FrmEditAlbaranVentaDet" );
/*>>>>>FRMEDITALBARANVENTADET_CONSTRUCTOR*/
	...
	editNLinea = addEditField( pControlsFrame, "ALBARANVENTADET", "NLINEA", preLayout );
	...
	editCantidad = addEditField( pControlsFrame, "ALBARANVENTADET", "CANTIDAD", cantidadLayout );
	...
	editPVPSinIVA = addEditField( pControlsFrame, "ALBARANVENTADET", "PVPSINIVA", preciosLayout );
	editPVP = addEditField( pControlsFrame, "ALBARANVENTADET", "PVP", preciosLayout );
	editDtoP100 = addEditField( pControlsFrame, "ALBARANVENTADET", "DTOP100", preciosLayout );
	editImporte = addEditField( pControlsFrame, "ALBARANVENTADET", "IMPORTE", preciosLayout );
	editImporteConIVA = addEditField( pControlsFrame, "ALBARANVENTADET", "IMPORTECONIVA", preciosLayout );
	editDescripcion = addTextField( pControlsFrame, "ALBARANVENTADET", "DESCRIPCION", descripcionLayout );
	editNotas = addTextField( pControlsFrame, "ALBARANVENTADET", "NOTAS", notasLayout );
...
void FrmEditAlbaranVentaDet::scatterFields()
{
    if( pFocusWidget == 0 )
        pFocusWidget = editCantidad;
    /*<<<<<FRMEDITALBARANVENTADET_SCATTER*/
	editNLinea->setText(getRecAlbaranVentaDet()->getValue("NLINEA").toInt());
	if( isEditing() && (pFocusWidget == 0) )
		pFocusWidget = editNLinea;
	editCantidad->setText(getRecAlbaranVentaDet()->getValue("CANTIDAD").toDouble());
	editPVPSinIVA->setText(getRecAlbaranVentaDet()->getValue("PVPSINIVA").toMoney());
	editPVP->setText(getRecAlbaranVentaDet()->getValue("PVP").toMoney());
	editDtoP100->setText(getRecAlbaranVentaDet()->getValue("DTOP100").toDouble());
	editImporte->setText(getRecAlbaranVentaDet()->getValue("IMPORTE").toMoney());
	editImporteConIVA->setText(getRecAlbaranVentaDet()->getValue("IMPORTECONIVA").toMoney());
	editDescripcion->setText(getRecAlbaranVentaDet()->getValue("DESCRIPCION").toString());
	editNotas->setText(getRecAlbaranVentaDet()->getValue("NOTAS").toString());
	scatterTipoIVA();
	scatterArticulo();
/*>>>>>FRMEDITALBARANVENTADET_SCATTER*/
    if ( isInserting() ) {
        double cli_dto = static_cast<FrmEditAlbaranVenta *>( getFrmMaster() )->getRecCliente()->getValue( "DTOP100" ).toDouble();
        editDtoP100->setText( cli_dto );
    }
    getRecArticulo()->addFilter( "ARTICULO.ENUSO=1" );
}

void FrmEditAlbaranVentaDet::gatherFields()
{
    /*<<<<<FRMEDITALBARANVENTADET_GATHER*/
	getRecAlbaranVentaDet()->setValue( "NLINEA", editNLinea->toInt());
	getRecAlbaranVentaDet()->setValue( "TIPOIVA_ID", getRecTipoIVA()->getRecordID() );
	getRecAlbaranVentaDet()->setValue( "CANTIDAD", editCantidad->toDouble());
	getRecAlbaranVentaDet()->setValue( "ARTICULO_ID", getRecArticulo()->getRecordID() );
	getRecAlbaranVentaDet()->setValue( "PVPSINIVA", editPVPSinIVA->toMoney());
	getRecAlbaranVentaDet()->setValue( "PVP", editPVP->toMoney());
	getRecAlbaranVentaDet()->setValue( "DTOP100", editDtoP100->toDouble());
	getRecAlbaranVentaDet()->setValue( "IMPORTE", editImporte->toMoney());
	getRecAlbaranVentaDet()->setValue( "IMPORTECONIVA", editImporteConIVA->toMoney());
	getRecAlbaranVentaDet()->setValue( "DESCRIPCION", editDescripcion->toString());
	getRecAlbaranVentaDet()->setValue( "NOTAS", editNotas->toString());
/*>>>>>FRMEDITALBARANVENTADET_GATHER*/
}

void FrmEditAlbaranVentaDet::scatterArticulo()
{
    /*<<<<<FRMEDITALBARANVENTADET_SCATTER_ARTICULO*/
	editArticuloCodigo->setText( getRecArticulo()->getValue("CODIGO") );
	editArticuloNombre->setText( getRecArticulo()->getValue("NOMBRE") );
/*>>>>>FRMEDITALBARANVENTADET_SCATTER_ARTICULO*/
}

void FrmEditAlbaranVentaDet::pushArticuloCodigo_clicked()
{
    /*<<<<<FRMEDITALBARANVENTADET_PUSH_ARTICULO_CODIGO_CLICKED*/
...
void FrmEditAlbaranVentaDet::scatterTipoIVA()
{
    /*<<<<<FRMEDITALBARANVENTADET_SCATTER_TIPOIVA*/
	editTipoIVACodigo->setText( getRecTipoIVA()->getValue("CODIGO") );
	editTipoIVANombre->setText( getRecTipoIVA()->getValue("NOMBRE") );
/*>>>>>FRMEDITALBARANVENTADET_SCATTER_TIPOIVA*/
}

void FrmEditAlbaranVentaDet::pushTipoIVACodigo_clicked()
{
    /*<<<<<FRMEDITALBARANVENTADET_PUSH_TIPOIVA_CODIGO_CLICKED*/
	char action = mControlKeyPressed;
...
void FrmEditAlbaranVentaDet::specialControlKeyPressed( QWidget *sender, char key )
{
    /*<<<<<FRMEDITALBARANVENTADET_SPECIALACTION*/
...
void FrmEditAlbaranVentaDet::validateFields( QWidget *sender, bool *isvalid, ValidResult *ir )
{
    /*<<<<<FRMEDITALBARANVENTADET_VALIDATE*/
	bool v=true;
	if( !isvalid )
		isvalid = &v;
	...
	if( validSeekCode( sender, isvalid, *validresult, editArticuloCodigo, editArticuloNombre,
		...
		scatterArticulo();
/*>>>>>FRMEDITALBARANVENTADET_VALIDATE*/
    /*<<<<<FRMEDITALBARANVENTADET_DETALLE_VALIDATE*/
	...
	double importesiniva =  cantidad * pvpsiniva * ( 1 - dtop100 / 100 );
		int tarifacliente = static_cast<FrmEditAlbaranVenta *>(getFrmMaster())->getRecCliente()->getValue("TARIFA").toInt();
...
void FrmEditAlbaranVentaDet::slotInsertTable_clicked()
{
    ModuleInstance->insertDetails( getFrmMaster(), this );
}

void FrmEditAlbaranVentaDet::updateStatus( bool callbehaviors )
{
