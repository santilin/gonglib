    ...
    if( beforetext == false ) {
        ...
        if ( object.borderStyle() != BorderNone ) {
            ...
            if ( object.borderTopStyle() != BorderNone ) {
                ...
                mPainter.setPen( linePen );
                mPainter.drawLine( x0, y0, x0 + width - object.borderRightWidth(), y0 );
            }
            if ( object.borderBottomStyle() != BorderNone ) {
                ...
                mPainter.setPen( linePen );
                mPainter.drawLine( x0, y0 + height - object.borderBottomWidth(), 
								   x0 + width - object.borderRightWidth(), y0 + height -object.borderBottomWidth() );
            }
            if ( object.borderLeftStyle() != BorderNone ) {
                ...
                mPainter.setPen( linePen );
                mPainter.drawLine( x0, y0, x0, y0 + height - object.borderBottomWidth() );
            }
            if ( object.borderRightStyle() != BorderNone ) {
                ...
                mPainter.setPen( linePen );
                mPainter.drawLine( x0 + width - object.borderRightWidth(), y0, 
								   x0 + width - object.borderRightWidth(), 
								   y0 + height - object.borderBottomWidth());
            }
        }
    }
    if( beforetext ) {
...
Measure ReportQtOutput::printObject( const Object &object )
{
    ...
    if( parent_section->padding() != 0.0 ) {
		...
		width -= parent_section->padding();
		height -= parent_section->padding();
    } else {
        x0 += parent_section->paddingLeft();
        y0 += parent_section->paddingTop();
		width -= parent_section->paddingLeft();
		height -= parent_section->paddingTop();
    }
    ...
    if( value.type() == Variant::tBinary ) {
        ...
        if ( text.size() ) {
            ...
            case RTK::AdjustGrow: {
                ...
                mPainter.drawText( x0, y0 /*+ fm.descent()*/, width, outrect.height(), tf, guitext );
