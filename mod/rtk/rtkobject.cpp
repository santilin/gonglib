...
{
    Xtring errmsg;
    UnitsType u = extractUnits(propPosX.getOrig(), repunit, errmsg);
    Measure m = stringTo<Measure>(propPosX.getOrig(), errmsg);
    bool posxwasminus1 = ( m == Measure(-1) );
    propPosX = fixOneMeasure( u, m, repunit, repsizex, outsizex );
    u = extractUnits(propPosY.getOrig(), repunit, errmsg);
    m = stringTo<Measure>(propPosY.getOrig(), errmsg);
    ...
    m = stringTo<Measure>(propSizeX.getOrig(), errmsg);
    if( m == Measure(0) ) {
        if( parent() )
    ...
    if( m == Measure(0) ) {
        ...
        u = extractUnits(propSizeY.getOrig(), repunit, errmsg);
        m = stringTo<Measure>(propSizeY.getOrig(), errmsg);
        propSizeY = fixOneMeasure( u, m, repunit, repsizey, outsizey);
    }
    u = extractUnits(propMarginLeft.getOrig(), repunit, errmsg);
    m = stringTo<Measure>(propMarginLeft.getOrig(), errmsg);
    propMarginLeft = fixOneMeasure( u, m, repunit, repsizex, outsizex );
    u = extractUnits(propMarginRight.getOrig(), repunit, errmsg);
    m = stringTo<Measure>(propMarginRight.getOrig(), errmsg);
    propMarginRight = fixOneMeasure( u, m, repunit, repsizex, outsizex );
    u = extractUnits(propMarginTop.getOrig(), repunit, errmsg);
    m = stringTo<Measure>(propMarginTop.getOrig(), errmsg);
    propMarginTop = fixOneMeasure( u, m, repunit, repsizey, outsizey );
    u = extractUnits(propMarginBottom.getOrig(), repunit, errmsg);
    m = stringTo<Measure>(propMarginBottom.getOrig(), errmsg);
    propMarginBottom = fixOneMeasure( u, m, repunit, repsizey, outsizey );
    if( isSection() ) {
        Section *sect = static_cast<Section *>(this);
        u = extractUnits(sect->propPadding.getOrig(), repunit, errmsg);
        m = stringTo<Measure>(sect->propPadding.getOrig(), errmsg);
        sect->propPadding = fixOneMeasure( u, m, repunit, repsizey, outsizey );
        u = extractUnits(sect->propPaddingLeft.getOrig(), repunit, errmsg);
        m = stringTo<Measure>(sect->propPaddingLeft.getOrig(), errmsg);
        sect->propPaddingLeft = fixOneMeasure( u, m, repunit, repsizey, outsizey );
        u = extractUnits(sect->propPaddingTop.getOrig(), repunit, errmsg);
        m = stringTo<Measure>(sect->propPaddingTop.getOrig(), errmsg);
...
/*<<<<<OBJECT_FIXPARAMETERS*/
    ...
    propAggregate.fix( parameters, delim, AggNone );
    propPosX.fix( parameters, delim, 0 );
    propPosY.fix( parameters, delim, 0 );
    propSizeX.fix( parameters, delim, 0 );
    propSizeY.fix( parameters, delim, 0 );
    propStyle.fix( parameters, delim );
    propUrl.fix( parameters, delim );
    propTag.fix( parameters, delim );
