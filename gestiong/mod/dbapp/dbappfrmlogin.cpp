...
{
#if 0
    /// \todo {0.4} Cambiar esto cuando manejemos mas drivers
    Xtring dbconn = ( getCreateDBDriver() == "MYSQL" ? "mysql" : "template1" );
    // Para crear una base de datos, primero hay que conectarse a una ya existente
    DBAPP->setRealSQLUser ( false );

    dbError createerror = DBAPP->dbOpen ( dbconn, getCreateDBDriver(), getCreateDBHost(), getCreateDBPort(),
                                          getCreateDBUser(), getCreateDBPassword() );
    if ( !createerror.isError() )
    {
        // creo la query aqui para que coja la base de datos recién abierta
        GSqlQuery * q = new GSqlQuery();
        try
        {
            // No hay manera de crear la base de datos y activarla en postgres
            if ( !q->existsDatabase ( mDBName ) )
            {
                if ( q->isPSQL() )
                    q->createDatabase ( mDBName, " WITH ENCODING 'LATIN1'" );
                else
                    q->createDatabase ( mDBName, "" );
            }
            delete q; // Va a cambiar la base de datos, la query ya no sirve
            createerror = DBAPP->dbOpen ( mDBName, getCreateDBDriver(), getCreateDBHost(), getCreateDBPort(),
                                          getCreateDBUser(), getCreateDBPassword() );
#endif
            try {
            ...
            else
            {
                ...
                case 1049: // Unknown database ....., but the user and password are right
                    detailedError = Xtring::printf( _("El usuario/a '%1s@%2s' no ha podido conectar con la base de datos '%3s' en %4s. "
                                                    ...
                                                    indexOf ( currentPage() ) == 0 ? getUser().c_str() : getCreateDBUser().c_str(),
                                                    indexOf ( currentPage() ) == 0 ? getHost().c_str() : getCreateDBHost().c_str(),
                                                    mDBName.c_str(),
