...
void GuiMainWindow::slotWindowStateChanged(Qt::WindowStates oldState, Qt::WindowStates newState)
{
    ...
    if( w ) {
        if(newState == Qt::WindowActive || newState == Qt::WindowMaximized ) {
            ...
            _GONG_DEBUG_PRINT(2, Xtring("Window: ") + w->name() + "(" + w->widget()->name() + ") activada." );
            if( !w->widget()->isEnabled() ) {
                _GONG_DEBUG_PRINT(2, Xtring( w->name() ) + " no está enabled" );
                if( pLastWindowDeactivated ) {
                    ...
                    _GONG_DEBUG_PRINT(2, Xtring("Voy a activar ") + pLastWindowDeactivated->name() );
...
// 						fb->restoreFocusWidget();
                ...
                _GONG_DEBUG_PRINT(2, Xtring("La ventana ") + w->name() + "(" + w->widget()->name() + ") estaba enabled" );
            }
        } else {
            pLastWindowDeactivated = w;
// 			if( FrmBase *fb = dynamic_cast<FrmBase *>(w->widget() ) )
// 				fb->saveFocusWidget();
            _GONG_DEBUG_PRINT(2, Xtring("Window ") + w->name() + " deactivated" );
        }
    } else {
