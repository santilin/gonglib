...
public:
    instruction() {
        data.opcode = op_invalid;
    }
    instruction(op_code op) {
        data.opcode = op;
    }
    instruction(string_table::entry ste) {
        data.strval = ste;
    }
    instruction(float_table::entry fte) {
        data.fltval = fte;
    }
    instruction(int i) {
        data.intval = i;
    }
    instruction(size_t off) {
        data.offset = off;
    }
    ...
    string_table::entry get_str() const {
        return data.strval;
    }
    float_table::entry get_flt() const {
        return data.fltval;
    }
    int get_int() const {
        return data.intval;
