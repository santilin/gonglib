...
context::context(RTK::Report *r) : log_out(0), report(r)
{
}
...
void context::log_msg(const std::string& message)
{
    if(log_out != 0)
...
value context::get_global(const std::string& name)
{
...
void context::set_global(const std::string& name,const value& val)
{
    if(name.length() > 0 && name[0] == '$')
        runtime.globals[runtime.strings.insert(name)] = val;
}

value context::get_local(const std::string& name)
{
    if(
...
void context::set_local(const std::string& name, const value& val)
{
    if(
        runtime.m_callstack.size() > 0 &&
        name.length() > 0 &&
...
value context::call(const std::string& func)
{
    args_t args;
    return call(func,args);
}

value context::call(const std::string& func,const args_t& args)
{
...
value context::eval(const std::string& code)
{
    try
    {
