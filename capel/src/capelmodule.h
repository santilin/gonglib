...
public:
    ...
    int remove_extrusion(const Xtring &extrusion_name, Xtring *parameters = 0);
    int clear_extrusion(const Xtring &extrusion_name);
    ...
    int new_extrusion(const Xtring &extrusion_name) { return clear_extrusion(extrusion_name); };
    ...
    Xtring get_current_extrusion() const { return mCurrentExtrusion; } 
	...
	static Xtring capitalize( const Xtring &str );
    Xtring::size_type find_global(const Xtring &search) const;
	int remove_empty_extrusions(const XtringList &extrusions);
	bool is_empty_extrusion(const Xtring &extrusion_name) const;
...
protected:
    ...
    Xtring::size_type lookup_extrusion_begin(const Xtring &extrusion_name) const;
    Xtring get_parameters(const Xtring &extrusion_name) const;
    Xtring::size_type lookup_extrusion_code(const Xtring &extrusion_name) const;
    Xtring::size_type lookup_extrusion_end(const Xtring &extrusion_name) const;
	// Adds a newline at the end in some circunstamces
    Xtring::size_type lookup_extrusion_end(const Xtring &extrusion_name);
    Xtring fill_begin_extrusion(const Xtring &extrusion_name) const;
    Xtring fill_begin_extrusion_with_space(const Xtring &extrusion_name) const;
    Xtring fill_end_extrusion(const Xtring &extrusion_name) const;
    Xtring fill_end_extrusion_with_space(const Xtring &extrusion_name) const;
